...
bool Polygon::Intersect(Ray ray, double tMin, double tMax, HitRecord &hr)
{
  SetNormal();
  hr.color = m_color;
  //cout << "Polygon color: " << m_color << endl;
  hr.kd = m_kd;//Diffuse coefficient
  hr.ks = m_ks;//Specular coefficient
  hr.e = m_e;//Specular exponent
  hr.kt = m_kt;//Refraction coefficient
  hr.it = m_it;//Index of refraction

  if(m_isConcave)
    {
      return IntersectConcave(ray, tMin, tMax, hr);
    }
  else
    {
      return IntersectConvex(ray, tMin, tMax, hr);
    }
}

bool Polygon::IntersectConvex(Ray ray, double tMin, double tMax, HitRecord &hr)
{
  //Vertices and edges
  Eigen::Vector3d v0 = m_vertices[0];
  Eigen::Vector3d v1 = m_vertices[1];
  ...
  //if phong shading is true, use the phong normal
  if(m_phong)
  {
      SetPhongNormal(m_alpha, m_beta, m_gamma);
  }
  hr.alpha = m_alpha;
  hr.beta = m_beta;
  hr.gamma = m_gamma;
  ...
  hr.point = point;
  hr.viewDir = -(point - ray.GetEye()).normalized();
...
bool Polygon::IntersectConcave(Ray ray, double tMin, double tMax, HitRecord &hr)
{
  Eigen::Vector3d v0 = m_vertices[0];
  Eigen::Vector3d v1 = m_vertices[1];
  Eigen::Vector3d v2 = m_vertices[2];
